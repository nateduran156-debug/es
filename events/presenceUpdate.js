...
export async function execute(oldPresence, newPresence) {
  ...
  for (const [groupName, vanityString] of Object.entries(vanityGroups)) {
    ...
    // Skip if they already had this vanity before — only log new detections
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({
        text: `vanity system | Today at ${timestamp}`,
      });

    await logChannel.send({
      content: isPingEnabled(groupName) ? "@everyone" : undefined,
      embeds: [embed],
